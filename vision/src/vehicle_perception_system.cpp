/**
 * @file vehicle_perception_system.cpp
 * @brief 车辆感知系统实现
 * @author pengchengkang
 * @date 2025-9-6
 * 
 * 本文件实现了车辆感知系统的核心功能，包括：
 * 1. 系统初始化：配置加载、模块初始化、线程池创建
 * 2. 生命周期管理：系统启动、停止、暂停、恢复等状态转换
 * 3. 帧处理流程：目标检测、跟踪、行为分析和结果处理
 * 4. 性能监控：实时统计系统各项性能指标
 * 5. 异常处理：完善的错误处理和日志记录机制
 * 
 * 实现特点：
 * - 采用多线程处理提高系统效率
 * - 使用线程池管理并发任务
